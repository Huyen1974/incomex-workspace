...
- **Tín hiệu actor theo từng việc:** `Vừa làm` = surface có commit gateway thành công gần nhất chạm đúng `work/<id>/`; đây là ký ức bền **không TTL**, giữ đến commit tiếp theo chạm chính task đó. Commit task khác không được thay tín hiệu này. Để giữ nguyên public MCP contract, gateway **không đổi Git author/message**: lớp transport ghi provenance nội bộ `(surface, commit SHA, work-id)` sau commit/push thành công; B2 chỉ gán actor khi commit mới nhất của task khớp provenance đó, nếu không thì `unknown`. `Đang làm` = presence tạm `(surface, work-id, last_seen)` từ tool activity scoped đúng task, TTL mặc định 10 phút; không đi qua Git, không tạo commit. Khi surface commit thành công task, presence của chính surface/task được clear và `Vừa làm` chuyển sang surface đó; presence surface khác trên cùng task không bị xoá. Xám = không có bằng chứng/tín hiệu gần đây, không khẳng định AI đã dừng. SSH/local ngoài gateway có thể không có `Đang làm`.
