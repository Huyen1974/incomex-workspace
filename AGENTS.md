...
- **Tín hiệu actor theo từng việc:** `Vừa làm` = surface có commit gateway thành công gần nhất chạm đúng `work/<id>/`; đây là ký ức bền **không TTL**, giữ đến commit tiếp theo chạm chính task đó. Commit task khác không được thay tín hiệu này. Gateway lấy identity client server-side và đặt **Git author name** bằng nhãn client đã sanitize; email vẫn là email cố định của gateway. B2 chỉ nhận actor khi author email là email gateway đã biết; commit SSH/human/legacy khác → `unknown`. Đây là metadata attribution của Git, không đổi MCP tool contract/schema. `Đang làm` = presence tạm `(surface, work-id, last_seen)` từ tool activity scoped đúng task, TTL mặc định 10 phút; không đi qua Git, không tạo commit. Khi surface commit thành công task, presence của chính surface/task được clear và `Vừa làm` chuyển sang surface đó; presence surface khác trên cùng task không bị xoá. Xám = không có bằng chứng/tín hiệu gần đây, không khẳng định AI đã dừng. SSH/local ngoài gateway có thể không có `Đang làm`.
- **Bảng phiên dịch người làm** (chỉ đổi tên hiển thị, không đổi identity): UI hiện tên theo dòng đầu tiên mà nhãn máy **bắt đầu bằng** cột 1; di chuột thấy nhãn máy gốc. Không khớp dòng nào → hiện nguyên nhãn (AI mới tự xuất hiện; biết là ai thì thêm một dòng vào bảng này, webhook tự đổ xuống). Script chỉ đọc bảng này, không giữ bảng thứ hai.
...
| `openai-mcp` | ChatGPT | ChatGPT; chế độ Agent/Work có thể trùng nhãn — chưa tách |
