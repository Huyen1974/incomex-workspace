...
- **Tín hiệu actor theo từng việc:** `Vừa làm` = surface có commit gateway thành công gần nhất chạm đúng `work/<id>/`; đây là ký ức bền **không TTL**, giữ đến commit tiếp theo chạm chính task đó. Commit task khác không được thay tín hiệu này. Gateway tự đặt Git author từ identity client server-side; author/prefix legacy dùng chung không được suy actor. `Đang làm` = presence tạm `(surface, work-id, last_seen)` từ tool activity scoped đúng task, TTL mặc định 10 phút; không đi qua Git, không tạo commit. Khi surface commit thành công task, presence của chính surface/task được clear và `Vừa làm` chuyển sang surface đó; presence surface khác trên cùng task không bị xoá. Xám = không có bằng chứng/tín hiệu gần đây, không khẳng định AI đã dừng. SSH/local ngoài gateway có thể không có `Đang làm`.
