...
- **HOST PREFLIGHT là cổng bắt buộc trước RUN.** Host chịu trách nhiệm bảo đảm agent có **đủ đầu vào và khả năng tiếp cận đầu vào từ chính môi trường agent sẽ chạy**, không được suy từ việc Host/MCP nhìn thấy dữ liệu ở một môi trường khác. Tối thiểu phải xác minh: (1) đúng repo/workspace + branch/ref/HEAD cần thiết; (2) agent đọc được `AGENTS.md`, `COLLAB.md`, `PROMPT.md` và mọi file/source bắt buộc; (3) các đường ngoài repo/attachment/connector cần dùng thực sự truy cập được từ agent; (4) tool/quyền đọc-ghi/exec cần thiết có sẵn; (5) đích ghi hợp lệ và không có lượt chạy xung đột. Nếu thiếu một mục thì **không phát RUN**.
- Nếu Host không thể kiểm trực tiếp môi trường thực thi của agent, chỉ được giao một lượt **PRECHECK chỉ đọc, không mutation** để agent trả về các bằng chứng trên. Host phải tự đối chiếu PRECHECK với SSOT/READY rồi mới phát RUN riêng. PRECHECK PASS không tự biến thành RUN.
