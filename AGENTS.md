...
- Định nghĩa dấu hiệu — máy đọc đúng chữ, không suy, **không phụ thuộc thứ tự dòng**: **A0** = dòng đầu tiên chứa `Xác nhận User:` kèm `ĐÃ XÁC NHẬN`/`CHƯA XÁC NHẬN`. **READY hợp lệ** = có một dòng chứa `READY@<40 hex>` không nằm trong `~~…~~` mà SHA trùng commit cuối chạm `work/<id>/PROMPT.md`; chuỗi `READY@` không đủ 40 hex (câu văn nhắc tới READY) không phải READY; READY cũ lệch SHA là trạng thái bình thường “đang điều chỉnh”, không cảnh báo. **RUN_ID hiện hành** = dòng `RUN_ID: <id>` trong `PROMPT.md`. **KQ** = dòng chứa `KQ@<RUN_ID hiện hành> XONG|DỪNG`; có cả hai thì `XONG` thắng. **Đã xong** = dòng dưới root `## Đã xong` chứa `` `work/<id>/` ``. **Chờ Owner** = dòng dưới `## Owner cần quyết` của việc không bắt đầu bằng `- —`; không còn gì chờ thì mục này chỉ còn đúng `- —`, điều đã quyết chuyển sang mục Quyết định Owner.
