...
- **Tín hiệu actor theo từng việc:** `Vừa làm` = surface có commit gateway thành công gần nhất chạm đúng thư mục hiện tại của việc (`work/<id>/` hoặc `work/done-tasks/<id>/` — việc Done hiện người vừa đóng); đây là ký ức bền **không TTL**, giữ đến commit tiếp theo chạm chính task đó. Commit task khác không được thay tín hiệu này. Gateway lấy identity client server-side và đặt **Git author name** bằng nhãn client đã sanitize; email vẫn là email cố định của gateway. B2 chỉ nhận actor khi author email là email gateway đã biết; commit SSH/human/legacy khác → `unknown`. Đây là metadata attribution của Git, không đổi MCP tool contract/schema. `Đang làm` = **một thanh ghi latest-only thật cho mỗi `work-id`**: `{surface, last_seen, generation}` từ activity scoped đúng task, TTL mặc định 10 phút. Tín hiệu mới hơn của bất kỳ surface nào **thay và vô hiệu vĩnh viễn trong generation đó** tín hiệu cũ của chính task, nên mỗi task tối đa một `Đang làm`. Record đã bị thay **không được sống lại** chỉ vì record mới bị clear/hết TTL. Không đi qua Git, không tạo commit. Khi surface đang là latest commit thành công task thì `Đang làm` của task được clear thành rỗng và `Vừa làm` chuyển sang surface đó; chỉ một activity mới sau thời điểm clear mới được làm sáng lại. Nếu đã có activity mới hơn của surface khác thì commit cũ không được clear tín hiệu mới. Xám = không có bằng chứng/tín hiệu gần đây, không khẳng định AI đã dừng. SSH/local ngoài gateway có thể không có `Đang làm`. Việc Done luôn không có `Đang làm`: gateway (đóng băng) hiểu `work/done-tasks/X` là `done-tasks`, nên publisher chỉ xuất `Đang làm` cho id có trong snapshot với bucket Now.
