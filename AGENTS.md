...
- **Tín hiệu actor theo từng việc:** `Vừa làm` = surface có commit gateway thành công gần nhất chạm đúng `work/<id>/`; đây là ký ức bền **không TTL**, giữ đến commit tiếp theo chạm chính task đó. Commit task khác không được thay tín hiệu này. Gateway lấy identity client server-side và đặt **Git author name** bằng nhãn client đã sanitize; email vẫn là email cố định của gateway. B2 chỉ nhận actor khi author email là email gateway đã biết; commit SSH/human/legacy khác → `unknown`. Đây là metadata attribution của Git, không đổi MCP tool contract/schema. `Đang làm` = **một thanh ghi latest-only cho mỗi `work-id`**: `{surface, last_seen}` từ activity scoped đúng task, TTL mặc định 10 phút; tín hiệu mới hơn của bất kỳ surface nào **thay ngay** tín hiệu cũ của chính task đó, nên mỗi task tối đa một `Đang làm`. Không đi qua Git, không tạo commit. Khi surface đang là latest commit thành công task thì `Đang làm` của task được clear và `Vừa làm` chuyển sang surface đó; nếu đã có activity mới hơn của surface khác thì commit cũ không được clear tín hiệu mới. Xám = không có bằng chứng/tín hiệu gần đây, không khẳng định AI đã dừng. SSH/local ngoài gateway có thể không có `Đang làm`.
...
|---|---|---|
| `Anthropic/ClaudeAI` | Claude Chat | App Claude (web/desktop/điện thoại) qua cổng Claude; Cowork có thể trùng nhãn — chưa tách |
| `claude-code` | Claude Code | Claude Code CLI |
| `codex` | Codex | Codex CLI / GPT Work |
| `openai-mcp` | GPT Chat | ChatGPT; chế độ Agent/Work có thể trùng nhãn — chưa tách |
