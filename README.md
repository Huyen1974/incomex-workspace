...
12. **TÀI LIỆU — GitHub/workspace là SSOT; VPS chỉ là Owner View/mirror.** Quy tắc này không thay đổi §11: MÃ/runtime tuyệt đối không đi từ GitHub xuống VPS.
    ...
    3. **Đồng bộ nền V1:** một bản sao workspace chỉ-đọc kiểm `main` mỗi 60 giây. Nếu remote HEAD không đổi so với revision đã xử lý thì thoát ngay, không dựng/chụp lại dữ liệu. Nếu HEAD đổi thì dựng snapshot từ HEAD mới. Nút **Cập nhật** của UI chỉ yêu cầu đọc lại snapshot hiện có/hoặc gọi cùng đường kiểm nếu hạ tầng sẵn có cho phép; không có pipeline thứ hai. Webhook để sau nếu độ trễ 60 giây thực tế không đủ.
    4. Snapshot phải gồm metadata task + HTML chính/asset được phép hiển thị; parser được đọc `COLLAB.md`/`PROMPT.md` nội bộ nhưng các file điều phối/evidence không được directory-list hay public/link mặc định. Task mới được tự phát hiện từ mọi `work/*/COLLAB.md`; root `## Đã xong` chỉ đánh dấu Done.
    5. **Last-good + lỗi:** mỗi lượt đồng bộ có lock; dữ liệu mới chỉ được publish sau khi dựng và validate xong toàn snapshot bằng thay thế nguyên tử hoặc cơ chế tương đương. Fetch/parse/copy lỗi → giữ nguyên bản tốt cuối, ghi trạng thái `error/stale`, tự thử lại ở lượt 60 giây kế tiếp; sự kiện/revision cũ không được ghi đè revision mới. Một task lỗi phải hiện warning/unknown nếu có thể, không làm mất toàn bộ dashboard.
    ...
    7. Mẫu đích: **tạo `work/<id>/COLLAB.md` → task tự xuất hiện → Git main đổi → tối đa khoảng 60 giây snapshot mới được dựng/publish → UI đang mở tự nạp dữ liệu mới**. Không `rsync --delete` cả repo; không tự chạy HTML/app code từ commit tài liệu.
