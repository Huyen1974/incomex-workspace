...
3. **Ba lệnh chuẩn — ghi vào AGENTS cùng commit đổi luật của RUN:** `Đóng <id>` = move `work/<id>/` → `work/done-tasks/<id>/`. `Mở lại <id>` = move ngược lại + Host chèn **khối A0 mới lên đầu §0** với `Xác nhận User: CHƯA XÁC NHẬN` ghi mục tiêu vòng mới (sửa/nâng cấp) → thanh Mục tiêu vàng, bắt đầu vòng mới đúng khung; giữ nguyên id và toàn bộ lịch sử trong cùng COLLAB (parser đọc dòng `Xác nhận User:` đầu tiên nên khối mới phải nằm trên). Owner gọi tên mơ hồ → AI tìm theo id/title/mục tiêu: đúng 1 kết quả thì làm và nhắc lại id; nhiều kết quả thì đề xuất 1 id kèm tên để Owner gật, không tự đoán.
