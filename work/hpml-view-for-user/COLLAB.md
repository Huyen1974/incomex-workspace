...
### KQ HVU-OWNERVIEW01-20260923-01 · Claude Code CLI · 23/09/2026
- Gate: `READY@25244942409cbcb4695b592091414174fb2804ab` = commit cuối chạm `PROMPT.md` ✔. Đường dùng: `fs_*` (ghi `ui`/`gh`, đọc `code`) + SSH **chỉ đọc** (journalctl, git log runtime). Không mutation runtime.
- **Việc 1 · ARCHIVED:** `ui:mow-mot-moit-mout.html` (sha256 `1e245ed6…99e8`, tạo ở ui `b889b29` 23/09 “Claude via MCP”, trùng byte HTML Git → bản ngoài §12) → `ui:archive/2026-09-23-ban-xem-sai-cho/mow-mot-moit-mout.html` + `INDEX.md` (ui `3ad9b0f`, `12dda6f`). `GET /ui-preview/mcp-writes/mow-mot-moit-mout.html` = 404; quét `ui` *.html: nội dung này chỉ còn ở bản archive. HTML chuẩn trong Git không chạm.
- **Việc 2 · chuỗi §12** (trước = sau, không sửa gì): (1) task `work/mow-mot-moit-mout/` có; (2) HTML chính `mow-mot-moit-mout.html`; (3) main `d49c6e9`; (4) journal `incomex-hvu-sync` 0 lượt lỗi từ 22/09, mỗi push publish sau 6–11 s; (5–6) `revisions/d49c6e9…/tasks.json`: task Now, `documentStatus=available`, `documentPath=/ui-preview/hpml-view-for-user/data/revisions/d49c6e90fdd3f23d1e543390e55613fcbcdfc881/documents/mow-mot-moit-mout/mow-mot-moit-mout.html`; (7) file đó 1.856.002 B, sha256 `7b6a315d…8587`; (8) Task view đọc `sync-status.json` → `tasksPath` đúng revision này, `fresh`; (9) FIELD01 `b53ce08` commit 06:25:21 → publish 06:25:27 · FIELD02 `9305a1d` 10:26:58 → 10:27:09 · FIELD03 `1ecb549` 11:47:41 → 11:47:47 (FIELD03 chỉ sửa COLLAB, HTML không đổi); (10) trình duyệt sạch không cookie mở link chuẩn: đúng task, `fresh`, tab Nội dung hiện HTML; request lỗi duy nhất `/api/directus/users/me` 401 của header KB (nút Login) → **không có cổng auth trên Task view** — không phải bảo vệ, không phải hồi quy. Auth giữ nguyên.
- **Root cause (một):** pipeline §12 không hỏng. `VIEW_PENDING_REVISION` sinh từ cách nghiệm thu của các lượt FIELD: bước “kiểm mirror” được chấm ngay trong commit báo cáo (snapshot tất yếu chỉ có sau commit ~6–11 s) và không ai kiểm lại sau publish; “đòi đăng nhập” là nút Login + 401 `users/me` của header, không phải cổng chặn. Evidence: journal trên, `data/sync-status.json`, `data/revisions/d49c6e9…/tasks.json`.
- Không chạy sync thủ công (đã `fresh` ở HEAD); không sửa mã/service/nginx/auth; không chạm MCP/connector/MMIM/presence. Hồi quy: `gsm-access-audit` (Now), `jev-integration` + `vps-clean-20-9-26` (Done) HTML publish sha256 = Git. `Cập nhật` = `window.location.reload()` của view (app.vue), không publish đường khác.
- **Lý do DỪNG — chỉ tiêu chí 6 (SHA-256 khớp Git):** HTML publish = đúng byte Git `1e245ed6…99e8` + **một khối `<script data-hvu-url-relay>` 688 B** chèn trước `</body>` (sha256 khối `debbf1e1…188d`; tiền tố/hậu tố trùng byte). Khối này do runtime commit `025b8fe` “[GPT] HVU · relay MMIM section URL on VPS” (nuxt-repo, 15:55 CEST 23/09; `sync.py` `with_mmim_url_relay`, chỉ áp task `mow-mot-moit-mout`) — không có trong PROMPT/COLLAB. Làm SHA khớp nguyên văn = sửa mã runtime → ngoài whitelist.
- **Owner chọn một:** (a) chấp nhận relay là lớp trình bày VPS → Host nghiệm thu bằng “SHA sau khi bỏ khối `data-hvu-url-relay` = Git” (đã đối chiếu PASS), không cần sửa gì; (b) gỡ relay: `git revert 025b8fe` trên nuxt-repo (2 file `scripts/hvu-b2/sync.py`, `ui/app.vue`), rebuild view theo README runtime, lượt sync kế tiếp cho SHA publish = Git; rủi ro: mất đồng bộ URL theo mục của MMIM; rollback = áp lại `025b8fe` (bản trước ở `/opt/incomex/deploys/hvu-url-relay-20260923/`).
- Owner View chuẩn: https://vps.incomexsaigoncorp.vn/knowledge/modules?task=mow-mot-moit-mout · JEV `gen-dec-1790173618-oDxbpukSRuUVOFqBo7gI` (login `no_gate` 1,00; verdict DỪNG 0,77).
...
## P35 · Claude Chat · Reviewer · ACCEPTED (Owner 24/09, áp runtime nuxt-repo `a7e933f`) — “GPT vừa sửa mà Task view không thấy”: tự kiểm 24/09
