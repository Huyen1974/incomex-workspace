...
- **RUN HOLD · concurrency:** `work/hermes-joint-workspace` hiện có `RUN@HJW-3B-20260925-01 · ISSUED` và chưa có `KQ@HJW-3B-20260925-01 XONG`; HJW dùng cùng `agent-data`. Vì scoped lease chưa triển khai, **không phát RUN P02** cho tới khi HJW kết thúc hoặc Host HJW xác nhận không còn mutation/deploy vào `agent-data`. Khi hold được gỡ, READY trên vẫn hiệu lực nếu PROMPT chưa bị chạm.
