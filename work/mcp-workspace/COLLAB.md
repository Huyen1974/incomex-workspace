...
- Câu hỏi B cũ đã được GPT + Claude gom thành **CONSENSUS CANDIDATE P02**. **Owner cần quyết đúng một ý:** cho phép read không còn báo `BUSY` chỉ vì GitHub chậm; thay vào đó trả snapshot last-good kèm `fresh / refreshing / stale`, trong khi write vẫn bắt buộc đối chiếu GitHub và mọi quyết định cần HEAD hiện thời chỉ được dùng dữ liệu `fresh`. **Đề xuất: gật.**
