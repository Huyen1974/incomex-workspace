...
## 1. Luật cứng
- **CẤM (D10/D12):** mọi `docker buildx …`, `docker builder …`, `docker system df|prune`, `docker image prune`, và mọi `-f`/`--force` khi xoá image. Docker chỉ được: `docker ps`, `docker image ls`, `docker inspect`, `docker info`, `docker version`, `docker logs`, `docker compose … config --images` (chỉ đọc, nội suy phía client — phần C), `docker image rm <repo:tag|ID>` (KHÔNG `-f`, chỉ phần C).
...
C1 Thêm luật (l) vào `scripts/vps-retention.sh` (chỉ thêm). **Tập GIỮ**, tính cho từng repository:
   ...
   2) mọi image được tham chiếu bởi cấu hình triển khai/rollback — **resolve thành ImageID thật trước khi lập danh sách xoá (P20)**. Nguồn quét: tệp triển khai dưới `/opt/incomex` (compose, `.env` của compose, manifest/rollback của deploy, script deploy) và unit trong `/etc/systemd/system`; liệt kê danh sách tệp/thư mục đã quét trong báo cáo. Ba loại tham chiếu, đều quy về ImageID bằng `docker image inspect`:
      ...
      **Fail-closed:** reference nào không resolve chắc chắn (biến thiếu, `config` lỗi, cú pháp lạ, tag không còn trên máy nhưng không xác định được repository…) → nếu biết repository thì **giữ toàn bộ image của repository đó** (không xoá gì trong repository đó lượt này); không biết repository → **bỏ toàn bộ lượt xoá** (lượt đầu: DỪNG phần C). Không đoán. Luật (l) làm lại toàn bộ bước resolve này mỗi lần chạy hằng tuần và ghi số reference chưa resolve vào dòng log;
