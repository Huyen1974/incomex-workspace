...
C1 Thêm luật (l) vào `scripts/vps-retention.sh` (chỉ thêm). **Tập GIỮ**, tính cho từng repository:
   ...
   2) mọi image được tham chiếu bởi cấu hình triển khai/rollback — **resolve thành ImageID thật trước khi lập danh sách xoá (P20)**. Nguồn quét: tệp triển khai dưới `/opt/incomex` (compose, `.env` của compose, manifest/rollback của deploy, script deploy) và unit trong `/etc/systemd/system`, **cộng tham chiếu gián tiếp (P23)**: với mỗi unit, theo `EnvironmentFile=`, `ExecStart=`/`ExecStartPre=`/`ExecStartPost=` (script/wrapper), và mọi symlink/tệp pointer/cấu hình mà unit hoặc script đó trỏ tới bằng đường dẫn cố định (đọc đích thật của symlink bằng `readlink -f`); lặp tới khi hết đường dẫn mới. Đường dẫn không đọc được, tính từ biến/lệnh động, hoặc không xác định chắc chắn → coi là reference chưa resolve (áp P22: giữ repository liên quan / bỏ lượt, R5 không XONG). Liệt kê danh sách tệp/thư mục đã quét (cả gián tiếp) trong báo cáo. Ba loại tham chiếu, đều quy về ImageID bằng `docker inspect --type=image <ref>` (đọc `.Id` / `.RepoDigests`):
   ...
   3) **2 ImageID khác nhau** mới nhất (theo Created) của mỗi repository, ngoài các ImageID đã giữ ở (1)(2) — đường lùi (P23: đếm theo ImageID, không theo tag; nhiều tag cùng trỏ một ImageID chỉ tính là 1 bản).
   Mọi tag trỏ vào một ImageID đã giữ cũng được giữ.
   ...
   **Ghi đúng 1 dòng** (cộng 1 dòng `(l) CANH_BAO …` chỉ khi có reference chưa resolve — P22) vào `/var/log/incomex/vps-retention.log`: số tag giữ/xoá/bỏ qua, số image còn lại, `du` kho image thật, `buildkit_du`. Thêm cờ `--chi-l` (chạy ngay) và `--chi-l-thu` (chỉ in danh sách sẽ xoá, không xoá).
   **Trần tính được — gọi đúng tên là "trần theo số version / working-set", KHÔNG phải hard-cap GB (P19):** mỗi repository ≤ (số ImageID đang dùng + số ImageID được tham chiếu + 2 ImageID). Báo cáo ghi GiB steady-state sau lượt xoá đầu.
C2 `bash -n`; thử khô với `docker` giả: nhánh thường; image đang dùng không bao giờ vào danh sách xoá; tag được tham chiếu không vào; image chỉ được tham chiếu bằng `repo@sha256:…` không vào; image chỉ được tham chiếu qua biến Compose (`image: repo:${TAG}` + `.env`) không vào; reference không resolve được → cả repository không bị xoá (hoặc bỏ lượt nếu không biết repository); giữ đúng 2 ImageID khác nhau mới nhất (có ca nhiều tag cùng trỏ 1 ImageID → vẫn còn 2 ImageID); tham chiếu chỉ nằm trong `EnvironmentFile=` / wrapper script / đích symlink của unit → không vào danh sách xoá; đường dẫn gián tiếp không đọc được → PENDING_UNRESOLVED; có tiến trình build → bỏ lượt; socket lỗi → `(l) LOI …` rc=0; cổng giờ/ngày đúng/sai. diff so với bản hiện hành (`45d5bc3`) chỉ có dòng thêm. Commit git cục bộ.
C3 Chạy thật `--chi-l-thu` → danh sách sẽ xoá vào hồ sơ + tóm tắt vào báo cáo (repository · số ImageID giữ · số tag giữ · số tag xoá). **Chốt an toàn trước lượt xoá đầu:** đối chiếu chéo lần hai — không ImageID nào của 12 container **và không ImageID nào trong tập tham chiếu đã resolve ở C1-2** nằm trong danh sách xoá; báo cáo ghi số reference theo loại (a)/(b)/(c) và số chưa resolve; số chưa resolve làm mất trần > 0 → `IMAGE = PENDING_UNRESOLVED` (P22), các repository đó không xoá và không được tính là có trần (kể cả ở các lượt tuần sau); mỗi repository còn ≥ 1 image sau xoá; sai → DỪNG phần C, không xoá.
C4 Chạy thật `--chi-l` 1 lần → 1 dòng log; `du` kho image trước/sau; 12 container StartedAt không đổi + health như cổng 3. (Image đã xoá không lùi được; đường lùi vận hành là 2 ImageID giữ lại mỗi repository + build lại từ mã. Lùi luật: `git revert` commit (l).)
