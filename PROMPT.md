...
## 2. Cổng năng lực chuẩn — 27 dòng, điều kiện trước deploy
Đây là danh mục **cố định theo nhu cầu người dùng**, không sinh từ lỗi đang gặp. Một bảng `# | Năng lực | GPT | Claude | Bằng chứng | PASS/FAIL/N/A(Dxx)` phải có đủ 27 dòng dưới đây, không ô trống. Còn FAIL (trừ N/A có Dxx) → không deploy.

1. LIST — root/path, pagination, Unicode path.
2. READ — cửa sổ nhỏ, dòng rất dài, tệp text vài MB.
...
6. LOG — lịch sử fresh, không bỏ commit mới.
7. DIFF — đúng path/version và fresh.
8. CREATE — tạo tệp ở nested path khi mọi parent chưa tồn tại; rollback dọn parent tự sinh.
9. EDIT — expected_version bắt buộc khi thay tệp; giữ byte ngoài vùng sửa; warning không biến thành block sai.
10. GUARDED REPLACE — thay toàn bộ tệp hiện hữu bằng expected_version, cả tệp nhỏ và HTML lớn; stale → reject, bản cũ nguyên.
...
15. TRANSACTION FILE+TREE — create/edit + copy/move file + copy/move directory; validate trước, đúng 1 commit, all-or-nothing.
...
24. UNICODE/NFC — giữ byte tên hiện hữu; D09 chặn tên mới NFC-equivalent khác byte; tên/path tiếng Việt thật PASS.
25. POLICY — gh chỉ UTF-8 text/HTML theo D11; binary gh N/A; secret scan không bị bypass; caps cỡ/tổng tài nguyên rõ ràng.
...
Ma trận G1–G16/C1–C16 của §13.11 vẫn phải xuất đầy đủ như evidence chi tiết; G16=N/A, không bỏ hàng. 27 dòng trên là **cổng cấp người dùng** để không còn phát hiện thiếu theo từng việc.
...
Chỉ khi 27/27 dòng PASS hoặc N/A có Dxx và G1–G16/C1–C16 không còn FAIL: gắn tag rollback cho image đang chạy mỗi bên; deploy agent-data + claude-mcp (+ helper nếu đổi); healthy trong 2 phút, không thì hoàn nguyên và DỪNG. Chạy lại `run_acceptance.py` hai bên → PASS. Không deploy lần hai trong R03 ngoài rollback.
...
Ghi nối tiếp KB §13.11: ma trận G1–G16/C1–C16, bảng 27 năng lực, deploy, flags client-contract, smoke mà Agent thực sự làm được. Chưa ghi KB thì không trả Owner.
