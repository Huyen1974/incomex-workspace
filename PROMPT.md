...
Schema/tool chỉ được đổi nếu thực sự cần để đóng năng lực #28 RESTORE; nếu đổi phải gộp toàn bộ trong **một lần cuối**, báo đủ tool/schema/metadata diff và `CLIENT_REFRESH_REQUIRED=YES`. Không đổi vì lý do khác.
...
9. EDIT — expected_version bắt buộc khi thay tệp; giữ byte ngoài vùng sửa, BOM/CRLF/LF/trailing spaces; warning không biến thành block sai.
...
25. POLICY/GIT SAFETY — gh chỉ UTF-8 text/HTML theo D11; binary gh N/A; secret scan không bị bypass; caps cỡ/path-segment/tổng tài nguyên rõ; main chỉ fast-forward, không force-push, không stage file ngoài thao tác.
...
28. HISTORICAL READ / SAFE RESTORE — phải lấy được một tệp text/HTML ở commit/version cũ và khôi phục **tệp hiện hữu** về bản đó bằng một forward commit mới, có expected current head/version, không reset/force, không relay tệp lớn qua model. Không được dùng restore để xoá tệp mới (quyền delete vẫn N/A). Cả GPT và Claude phải PASS; nếu cần đổi schema/tool thì dùng quyền đổi-một-lần của mục 1.
