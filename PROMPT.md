...
1. Kiểm READY trên bản sao riêng (repo công khai, không cần khoá): `git clone --depth 50 https://github.com/Huyen1974/incomex-workspace /tmp/iw-ready`; so `git log -1 --format=%H -- PROMPT.md` với dòng `READY@...` trong `COLLAB.md`. Khớp tiền tố → làm; lệch hoặc chưa có READY → DỪNG, báo.
...
| H05 | GPT | `git diff --cached --check` là chốt chặn cứng (write ~446, transaction ~110): dòng trống cuối file, 2 dấu cách xuống dòng Markdown bị từ chối | CHỐT của Founders: chuyển thành CẢNH BÁO không chặn, trả `warnings` kèm path:line:lý do; không bao giờ tự sửa bytes | P0 |
...
## 3. Thứ tự và điểm dừng an toàn
- Làm P0 → P1 → P2. Xong mỗi mục: test xanh + commit mã vào git sẵn có của thư mục mã, message `[Claude Code] Hxx · <tóm tắt>`.
...
- Ngữ cảnh chạm 60–70%: dừng ở ranh giới một mục đã xong, ghi bàn giao vào báo cáo (xong gì · dở gì · làm tiếp từ đâu). Không để mã nửa chừng chưa commit hoặc chưa deploy.
...
- C. Lỗi minh bạch: nội dung có dấu cách cuối dòng và dòng trống cuối file → ghi THÀNH CÔNG kèm `warnings`, bytes giữ nguyên; `expected_version` cũ; remote đã chạy trước; lỗi git giả lập → đúng phase, không còn `GIT_OPERATION_FAILED` chung khi đã biết nguyên nhân.
